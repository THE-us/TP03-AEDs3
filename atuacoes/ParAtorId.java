...
public class ParAtorId implements RegistroArvoreBMais<ParAtorId> {
    ...
    public ParAtorId () throws Exception {
        this ("", -1);
    }

    public ParAtorId (String n) throws Exception {
        this (n, -1);
    }

    public ParAtorId (String t, int i) throws Exception {
        ...
        if (!t.isEmpty ()) {
            ...
            if (vb.length > TAMANHO_NOME) {
                ...
                // Verifica se os últimos bytes estão fora do intervalo de 0 a 127 (o que
                // indicaria que o último caractere é um caractere acentuado)
                ...
                // Cria uma nova string para o título a partir desse vetor de no máximo
                // TAMANHO_TITULO bytes
                t = new String (vb2);
            }
        }
    ...
    @Override
    public ParAtorId clone () {
        try {
            return new ParAtorId (this.nome, this.id);
        } catch (Exception e) {
            e.printStackTrace ();
        }
        return null;
    ...
    public int compareTo (ParAtorId a) {
